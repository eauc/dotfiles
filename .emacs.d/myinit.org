...
** Global keybindings
   ...
   <Fn> keys.
   #+BEGIN_SRC emacs-lisp
   (global-set-key (kbd "C-v")  'hydra-main/body)
...
** Moves
   ...
   (use-package cool-moves
     ...
     :bind (("C-M-d" . 'cool-moves/paragraph-forward)
            ("C-M-u" . 'cool-moves/paragraph-backward)
            ("C-M-z" . 'cool-moves/line-forward)
            ("C-M-w" . 'cool-moves/line-backward)
            ("C-M-s" . 'cool-moves/word-forward)
            ("C-M-a" . 'cool-moves/word-backwards)))
   #+END_SRC
...
** Multi-mode
   ...
   Enable multiple mode in buffer regions.
   ...
   (use-package mmm-mode
     ...
     :ensure t
     :custom
     (mmm-global-mode 'maybe "Load mode when file match a rule")
     (mmm-parse-when-idle t "Refresh MMM regions when file is modified")
     (mmm-submode-decoration-level 0 "Hides background color in MMM regions")
     :config
     (require 'mmm-auto)
     (mmm-add-classes
      '((vue-template
        ...
        (vue-style
         ...
         :front "<style[^>]*>"
         :back "</style>")))
     (mmm-add-mode-ext-class nil "\\.vue\\'" 'vue-template)
     (mmm-add-mode-ext-class nil "\\.vue\\'" 'vue-script)
     (mmm-add-mode-ext-class nil "\\.vue\\'" 'vue-style))
   #+END_SRC
...
** Flycheck
   ...
   Flycheck pos-tip: displays flycheck errors as tooltips
   (prevents conflict with mode bar messages)
   #+BEGIN_SRC emacs-lisp
   ...
   (use-package graphql-mode
     ...
     (mmm-add-classes
      '((js-graphql
         ...
         :front "[^a-zA-Z]gql`"
         :back "`")))
     (mmm-add-mode-ext-class nil "\\.js\\'" 'js-graphql)
     (mmm-add-mode-ext-class nil "\\.vue\\'" 'js-graphql))
   #+END_SRC
...
** Typescript
   ...
   (use-package tide
     ...
     :ensure t
     :hook
     ((typescript-mode . eauc/setup-tide-mode)
